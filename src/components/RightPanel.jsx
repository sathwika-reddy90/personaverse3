...
export default function RightPanel({ className = '' }) {
  ...
  if (!results) {
    return (
      <aside className={`w-80 shrink-0 flex-col border-l border-ink/[0.06] px-5 py-6 ${className}`}>
  ...
  return (
    <aside className={`w-80 shrink-0 flex-col border-l border-ink/[0.06] px-5 py-6 overflow-y-auto space-y-5 ${className}`}>
      <div>
      ...
      <GlassCard plain className="p-5 bg-gradient-to-br from-ink to-primary text-white border-0">
        ...
        <button onClick={() => navigate('/insights')} className="text-[11px] font-bold text-highlight">
